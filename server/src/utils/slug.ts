...
const locales = JSON.parse('{"bg":{"Й":"Y","Ц":"Ts","Щ":"Sht","Ъ":"A","Ь":"Y","й":"y","ц":"ts","щ":"sht","ъ":"a","ь":"y"},"de":{"Ä":"AE","ä":"ae","Ö":"OE","ö":"oe","Ü":"UE","ü":"ue","ß":"ss","%":"prozent","&":"und","|":"oder","∑":"summe","∞":"unendlich","♥":"liebe"},"es":{"%":"por ciento","&":"y","<":"menor que",">":"mayor que","|":"o","¢":"centavos","£":"libras","¤":"moneda","₣":"francos","∑":"suma","∞":"infini","♥":"amor"},"fr":{"%":"pourcent","&":"et","<":"plus petit",">":"plus grand","|":"ou","¢":"centime","£":"livre","¤":"devise","₣":"franc","∑":"somme","∞":"infini","♥":"amour"},"pt":{"%":"porcento","&":"e","<":"menor",">":"maior","|":"ou","¢":"centavo","∑":"soma","£":"libra","∞":"infinito","♥":"amor"},"uk":{"И":"Y","и":"y","Й":"Y","й":"y","Ц":"Ts","ц":"ts","Х":"Kh","х":"kh","Щ":"Shch","щ":"shch","Г":"H","г":"h"},"vi":{"Đ":"D","đ":"d"},"da":{"Ø":"OE","ø":"oe","Å":"AA","å":"aa","%":"procent","&":"og","|":"eller","$":"dollar","<":"mindre end",">":"større end"},"nb":{"&":"og","Å":"AA","Æ":"AE","Ø":"OE","å":"aa","æ":"ae","ø":"oe"},"it":{"&":"e"},"nl":{"&":"en"},"sv":{"&":"och","Å":"AA","Ä":"AE","Ö":"OE","å":"aa","ä":"ae","ö":"oe"}}');
...
/**
 * Convert title to slug using custom slugify implementation with Chinese support
 * 
 * Logic:
 * 1. Transliterate (e.g. 'é' -> 'e', '&' -> 'and')
 * 2. Keep Alphanumeric (\w), Spaces, Hyphens, and Chinese (\u4e00-\u9fa5)
 * 3. Remove everything else
 * 4. Lowercase
 */
export function toArticleSlug(string: string, options?: SlugifyOptions | string): string {
    ...
    // Use specific locale if given, otherwise empty object
    const localeMap = (opts.locale && locales[opts.locale]) ? locales[opts.locale] : {};

    const replacement = opts.replacement === undefined ? '-' : opts.replacement;
    const trim = opts.trim === undefined ? true : opts.trim;
    const lower = opts.lower === undefined ? true : opts.lower; // Default to true per requirement
    const strict = opts.strict === undefined ? false : opts.strict;
    // Default remove regex to keep Chinese if not provided
    const removeRegex = opts.remove || /[^\w\s\u4e00-\u9fa5\-]+/g;

    let slug = string.normalize().split('')
        // replace characters based on charMap
        .reduce(function (result, ch) {
            // 1. Check locale specific map
            let appendChar = localeMap[ch as keyof typeof localeMap];
            // 2. Check general char map
            if (appendChar === undefined) appendChar = charMap[ch as keyof typeof charMap];
            // 3. Keep original if no match
            if (appendChar === undefined) appendChar = ch;

            // 4. Replace occurrences of the replacement char with space (to define separators later)
