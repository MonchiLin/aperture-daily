                        ...
                        {displayArticles.map((article, idx) => (
                            <article key={article.id} className="group relative border-b border-stone-200 last:border-0 hover:bg-stone-50 -mx-4 px-4 transition-colors cursor-pointer">
                                <a href={`/article/${article.id}`} className="flex items-center py-3 w-full gap-4">
                                    {/* Number */}
                                    <span className="inline-flex items-center text-xs font-bold text-stone-300 font-sans w-6 shrink-0 group-hover:text-stone-500 transition-colors">
                                        {String(idx + 1).padStart(2, '0')}
                                    </span>

                                    {/* Title */}
                                    <h3 className="inline-flex items-center flex-1 font-serif text-lg font-medium text-slate-900 group-hover:text-amber-900 transition-colors truncate pr-4">
                                        {article.title}
                                    </h3>
